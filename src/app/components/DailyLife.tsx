...
export function DailyLife() {
    return (
        <section id="daily" className="py-24 px-4">
            <div className="max-w-4xl mx-auto">
                <Animated animation="fade-up">
                    <h2 className="text-4xl md:text-5xl text-center mb-4 text-glow-warm font-bold">A Day in My Life</h2>
                </Animated>
                    ...
                    <div className="space-y-8">
                        {schedule.map(({ time, icon: Icon, title, desc, color, side }, i) => (
                            <Animated key={time} animation={side === 'left' ? 'fade-right' : 'fade-left'} delay={(i % 3 * 100) as 0 | 100 | 200}>
                                <div className={`flex items-center gap-4 ${side === 'right' ? 'md:flex-row-reverse' : 'md:flex-row'} flex-row`}>
                                    {/* Card */}
                                    <div className="flex-1 glass glass-card-hover p-5 rounded-2xl relative" style={{ border: `1px solid ${color}30` }}>
                                    ...
                                    {/* Center dot (desktop) */}
                                    <div className="hidden md:flex w-4 h-4 rounded-full flex-shrink-0 border-2 z-10"
                ...
                <Animated animation="zoom-in" delay={200}>
                    <div className="mt-14 glass-strong p-8 text-center">
                        <p className="text-lg" style={{ color: 'var(--text-body)', lineHeight: 1.8 }}>
