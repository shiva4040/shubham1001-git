...
const funFacts = [
  {
    icon: Coffee, title: 'Power Cut Expert',
    ...
    gradient: 'linear-gradient(135deg, rgba(251,191,36,0.7), rgba(245,158,11,0.5))', glow: 'rgba(251,191,36,0.45)', iconColor: '#fde68a', delay: 0 as const,
  },
  {
    icon: Lightbulb, title: 'Random Fact Collector',
    ...
    gradient: 'linear-gradient(135deg, rgba(6,182,212,0.7), rgba(59,130,246,0.5))', glow: 'rgba(6,182,212,0.45)', iconColor: '#67e8f9', delay: 200 as const,
  },
  {
    icon: BookOpen, title: 'The Overthinker',
    ...
    gradient: 'linear-gradient(135deg, rgba(168,85,247,0.7), rgba(236,72,153,0.5))', glow: 'rgba(168,85,247,0.45)', iconColor: '#d8b4fe', delay: 100 as const,
  },
  {
    icon: Smile, title: 'Village Life Perks',
    ...
    gradient: 'linear-gradient(135deg, rgba(16,185,129,0.7), rgba(5,150,105,0.5))', glow: 'rgba(16,185,129,0.45)', iconColor: '#6ee7b7', delay: 300 as const,
  },
];
...
const quotes = [
  {
    ...
    border: 'rgba(99,102,241,0.5)', bg: 'rgba(99,102,241,0.12)', delay: 0 as const,
  },
  {
    ...
    border: 'rgba(168,85,247,0.5)', bg: 'rgba(168,85,247,0.12)', delay: 100 as const,
  },
  {
    ...
    border: 'rgba(16,185,129,0.5)', bg: 'rgba(16,185,129,0.12)', delay: 200 as const,
  },
];
    ...
    <section className="py-16 md:py-24 px-4">
      <div className="max-w-6xl mx-auto">
        <Animated animation="fade-up">
          <h2 className="text-3xl sm:text-4xl md:text-5xl text-center mb-3 md:mb-4 text-glow font-bold">Fun Facts About Me 😄</h2>
        </Animated>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6 mb-8 md:mb-12">
          {funFacts.map((fact) => (
            <Animated key={fact.title} animation="fade-up" delay={fact.delay}>
              <div className="glass glass-card-hover group p-5 md:p-6 h-full">
                <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl flex items-center justify-center mb-3 md:mb-4 transition-transform duration-300 group-hover:scale-110"
                  style={{ background: fact.gradient, boxShadow: `0 0 20px ${fact.glow}`, border: '1px solid rgba(255,255,255,0.18)' }}>
                  <fact.icon className="w-5 h-5 md:w-6 md:h-6" style={{ color: fact.iconColor }} />
                </div>
                <h3 className="text-lg md:text-xl mb-2 font-semibold" style={{ color: 'var(--text-heading)' }}>{fact.title}</h3>
                <p className="text-sm md:text-base" style={{ color: 'var(--text-body)', lineHeight: 1.75 }}>{fact.description}</p>
              </div>
            </Animated>
          ))}
        </div>
          ...
          {quotes.map(({ text, border, bg, delay }) => (
            <Animated key={text} animation="fade-left" delay={delay}>
              <div className="p-4 md:p-6 rounded-2xl" style={{
                ...
                borderLeftWidth: '4px', borderLeftColor: border, backdropFilter: 'blur(12px)',
              }}>
                <p className="italic text-sm md:text-base" style={{ color: 'var(--text-quote)' }}>{text}</p>
              </div>
            </Animated>
          ))}
        </div>
        ...
        <Animated animation="zoom-in" delay={100}>
          <div className="glass-strong p-6 md:p-8 text-center aurora-bg">
            <h3 className="text-xl md:text-2xl mb-3 md:mb-4 font-semibold" style={{ color: 'var(--text-heading)' }}>Life as a 7th Grader 📚</h3>
            <p className="text-sm md:text-lg max-w-3xl mx-auto" style={{ color: 'var(--text-body)', lineHeight: 1.8 }}>
            ...
            <div className="mt-4 md:mt-6 text-3xl md:text-4xl">🚀✨🎮🔬</div>
          </div>
        </Animated>
      </div>
    </section>
